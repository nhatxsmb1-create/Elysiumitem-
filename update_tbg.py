...
tbg = '''package dev.elysium.item.gui;
...
public class TrangBiGui extends ElysiumGui {
    ...
    public void build(Player player) {
        ...
        // Lõi Hệ Phái - Slot 13
        setButton(13, new GuiButton(getSlotItem(slotData.getCore(), "Lõi Kỹ Năng", Material.NETHER_STAR), e -> {
            e.setCancelled(true);
            handleSlotClick(player, AccessorySlotData.SlotType.CORE, "Lõi Kỹ Năng", slotData);
        }));
    ...
    private void handleSlotClick(Player player, AccessorySlotData.SlotType slotType, String requiredSubType, AccessorySlotData slotData) {
        ...
        if (currentEquipped != null) {
            ...
            if (slotType == AccessorySlotData.SlotType.CORE) {
                try {
                    Class<?> apiClass = Class.forName("dev.elysium.combat.api.CombatAPI");
                    java.lang.reflect.Method unequip = apiClass.getMethod("unequipCore", Player.class);
                    unequip.invoke(null, player);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
...
'''
write_file('src/main/java/dev/elysium/item/gui/TrangBiGui.java', tbg)
print("Updated TrangBiGui.java")